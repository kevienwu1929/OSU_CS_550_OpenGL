...
float Ta = 0., Tb = 0., Tc = 0., Ts = 0., Tt = 0.;
...
Display( )
{
	...
	{
		uA = cos ( 36 * Time / M_PI+ AMP);
		uB = sin ( 36 * Time /M_PI+ AMP);
		uC = cos ( 36 * Time * 2 /M_PI+ AMP);

		Ta = uA;
		Tb = uB;
		Tc = uC;
	
	}else if( Ta != 0 && Tb != 0 && Tc != 0){
	...
	{
		S0 = 0.5 + sin (36 * Time / M_PI);
		T0 = 0.5 + cos (36 * Time / M_PI);
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'V':
		case 'v':
			ImUsingTheVertexShader = !ImUsingTheVertexShader;
			break;

		case 'F':
			ImUsingTheFragmentShader = !ImUsingTheFragmentShader;
			break;
