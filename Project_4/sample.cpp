...
bool    Frozen;					// True = Freeze everything
GLuint  DL;
GLuint  Man;
...
Display( )
{
	...
	glRotatef( 360.*Time *2, 0., 0., 1. );
	
	SetMaterial(0, 0, 1, 8);
	glTranslatef(0, 3, 0);
	SetPointLight(GL_LIGHT1, 0, 0, -1, 0, 0, 1);
	MjbSphere(0.05, 100, 100);
	glCallList( DL );
	glPopMatrix( );
	...
	glPushMatrix( );
	SetSpotLight(GL_LIGHT2, 0, 0, 4, 0, 0, -1, 1, 0, 0);
	glTranslatef(0, 0, 4);
	SetMaterial(1, 0, 0, 3);
	MjbSphere(0.05, 100, 100);
	glPopMatrix( );
...
InitLists( )
{
	...
	DL = glGenLists( 1 );
		...
		SetMaterial(0, 1, 1, 8);
		glColor3f( 0., 1., 1. );
		LoadObjFile( "tinker.obj" );
		
	glEndList( );

	Man = glGenLists( 1 );
		glNewList( Man, GL_COMPILE );
